...
for v = 1:count
    for i = 1:length(GAM)
        for j = 1:length(GAM{i})
            ...
            [struktura, matrix] = PaieskaIPloti(GAM, v, count);
            is = false;
        
            for z = 1:length(arrayOfMatrix)
                if isequal(arrayOfMatrix{z}, matrix) || isequal(arrayOfMatrix{z},matrix.')
                    is = true;
                    break;
                end
            end
...
figure(2)
title('Gautasis grafas')
plotGraphVU1(V,atsMas{30},orgraf,arc,Vkor,poz,Fontsize,storis,spalva)

function [struktura, atsMas] = PaieskaIPloti(GAM, v, count)

package scalatron.botwar.botPlugin.Arunas
...
object ControlFunction
{
    def forMaster(bot: Bot) {
        ...
        //Spawns resource selector
        if (bot.energy > 100 && bot.time % 100 > 24) {
            bot.spawn(XY(1, 0), "mood"-> "Hungry")
           }
           else {
               if(bot.energy > 700 && bot.time % 74 > 74 && rand.nextInt(4) > 1){
                   bot.spawn(XY(1,1), "mood" -> "Decoy")
                   bot.say("opa")
               }
           }
    }
    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("mood", "Lurking") match {
            case "Hungry" => reactAsSelectorBot(bot)
            case "Decoy" => reactAsDecoybot(bot)
    ...
    def reactAsSelectorBot(bot: MiniBot) {
        val BFS = bot.view.BFS match {
        case None => bot.offsetToMaster.signum
        case Some(delta: XY) => delta.signum
       }
       bot status("Niam Niam")
       bot.move(BFS)
    }

    def reactAsDecoybot(bot: MiniBot){
        bot status("I'm Master")
    }
    ...
    def BFS() = {
        ...
        while(!foodItem && !QueueV.isEmpty){
            ...
            if (!foodItem) {
                ...
                {
                    i = i+1;
                    if (v.x + X(i) > 0 && v.x + X(i) < size && v.y + Y(i) > 0 && v.y + Y(i) < size && visited(XY(v.x + X(i), v.y + Y(i))) == false) {
